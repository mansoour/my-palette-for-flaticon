/**
 * A small set of Bootstrap Icons (MIT licensed, https://icons.getbootstrap.com/), embedded
 * inline so nothing is fetched at runtime — required for a Manifest V3 extension anyway (no
 * remote code/assets), and it keeps everything working offline.
 *
 * Loaded as a plain script (like storage.js) by the content script, popup, and dashboard.
 */
const FPMIcons = (function () {
  const PATHS = {
    palette:
  ...
  /** Returns an inline `<svg>` markup string for template literals / innerHTML. */
  function svg(name, { size = 16, className = "" } = {}) {
    const inner = PATHS[name];
    if (!inner) return "";
    const cls = "fpm-icon" + (className ? " " + className : "");
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" fill="currentColor" viewBox="0 0 16 16" class="${cls}" aria-hidden="true" focusable="false">${inner}</svg>`;
  }

  /** Returns a detached SVG Element, for code that builds UI via createElement/appendChild. */
  function create(name, opts) {
    const wrap = document.createElement("div");
    wrap.innerHTML = svg(name, opts).trim();
...
if (typeof window !== "undefined") {
  window.FPMIcons = FPMIcons;
}
if (typeof module !== "undefined") {
  module.exports = FPMIcons;
}
